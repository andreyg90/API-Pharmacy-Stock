...
router.post('/',[
    ...
    check('expirationDate','La fecha de vencimiento es obligatoria').not().isEmpty(),
    check('status','El estado del medicamento es obligatorio').not().isEmpty(),
    validarCampos],createMedicine)
router.put('/:id',[
    check('id','No es un id válido').isMongoId(),
    check('id').custom(isMedicineById),
    validarCampos],updateMedicine)
router.delete('/:id',[
    check('id','No es un id válido').isMongoId(),
    check('id').custom(isMedicineById),
    validarCampos],deleteMedicine)
router.post('/sellMedicine/:id',[
    check('id','No es un id válido').isMongoId(),
